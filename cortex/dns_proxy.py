...
class TelosDNSProxy:
    def __init__(self, ipc_client, host='127.0.0.1', port=5353, upstream_dns='8.8.8.8', max_workers: int = 20):
        self.host = host
        self.port = port
        ...
        # Cleanup queue for DNS TTL expiration events.
        # Keyed by (ip_int, auth_id) so repeated authorizations for the same
        # IP/domain extend an existing cleanup instead of stacking timers.
    ...
    def _release_ip(self, ip_int, auth_id):
        ...
        with self._ip_refcount_lock:
            ...
            if entry['auth_ids']:
                ...
                log.debug(
                    f"[DNS] IP {ip_int}: released auth_id={auth_id}, refcount={len(entry['auth_ids'])}, next_expiry={entry['max_expiry_ts']:.1f}"
                )
                return False
    ...
    def _schedule_drawbridge_cleanup(self, ip_int, ip_str, domain, ttl_seconds):
        ...
        with self._cleanup_lock:
            if len(self._pending_cleanups) >= MAX_PENDING_CLEANUPS and key not in self._pending_cleanups:
                log.error(
                    f"[DNS] Cleanup queue full ({len(self._pending_cleanups)} pending), dropping cleanup for IP={ip_int} domain={domain}"
                )
                return
            ...
            if old_expiry is None:
                ...
                self._pending_cleanups[key] = expiry
                log.debug(
                    f"[DNS] Extended cleanup for IP={ip_int} domain={domain} {old_expiry:.0f}→{expiry:.0f}"
                )
            else:
                log.debug(
                    f"[DNS] Cleanup unchanged for IP={ip_int} domain={domain} existing={old_expiry:.0f} new={expiry:.0f}"
                )
        ...
        while not self._stop_event.is_set():
            ...
            for ip_int, auth_id in expired:
                if self._release_ip(ip_int, auth_id):
                    ...
                    active_domains = self._active_domains_for_ip(ip_int)
                    log.debug(
                        f"[DNS] IP {ip_int}: cleanup deferred, active domains={active_domains}"
                    )
